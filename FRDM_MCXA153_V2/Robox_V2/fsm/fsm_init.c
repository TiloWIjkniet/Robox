...
int FSM_init(void)
{
    ...
    FSM_addState(S_RESET,           &(state_funcs_t){reset_onEntry,     reset_onUpdate,     reset_onExit});
    ...
    FSM_addTransition(&(transition_t){S_IDLE,       E_START_GAME,            S_FIRST_ROOM}); 
    FSM_addTransition(&(transition_t){S_FIRST_ROOM, E_ROOM_COMPLETED,        S_ROOM_LOOP});  
    FSM_addTransition(&(transition_t){S_ROOM_LOOP,  E_ROOM_COMPLETED,        S_ROOM_LOOP});  
    FSM_addTransition(&(transition_t){S_ROOM_LOOP,  E_ROOM_LOOP_TO_LAST,     S_LAST_ROOM});  
    FSM_addTransition(&(transition_t){S_LAST_ROOM,  E_LAST_ROOM_COMPLETED,   S_COMPLETED});  
    ...
    FSM_addTransition(&(transition_t){S_FIRST_ROOM, E_ROOM_TIMEOUT,          S_TIMEOUT});    
    FSM_addTransition(&(transition_t){S_ROOM_LOOP,  E_ROOM_TIMEOUT,          S_TIMEOUT});    
    FSM_addTransition(&(transition_t){S_LAST_ROOM,  E_ROOM_TIMEOUT,          S_TIMEOUT});    
    FSM_addTransition(&(transition_t){S_TIMEOUT,    E_GAME_TIMEOUT,          S_RESET});      

    FSM_addEvent(E_INIT_COMPLETE);
    return 0;
}